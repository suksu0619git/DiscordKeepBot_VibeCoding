...
| `OT_NOTICE_CHANNEL_ID` | `1405484426799874109` | 신입 OT 수요조사 공지 채널 (📢ㆍ공지) |
| `OT_NOTICE_WEEKDAY` | `2` | 공지 요일 (0=월 … 2=수 … 6=일) |
| `OT_NOTICE_HOUR` / `OT_NOTICE_MINUTE` | `14` / `0` | 공지 시각 (KST) |
| `OT_NOTICE_TITLE` | `🐣 신입 오리엔테이션 수요조사` | 공지 임베드 제목 |
...
| `OT_DEADLINE_HOUR` / `OT_DEADLINE_MINUTE` | `19` / `0` | 수요조사 마감 공지 시각 |
| `OT_DEADLINE_CHANNEL_ID` | (일정 알림 채널) | 마감 공지를 올릴 채널 |
| `OT_DEADLINE_PLACE` | `Studio KEEP KEXCO` | 진행 공지에 넣을 장소 |
| `OT_DEADLINE_OPEN_MESSAGE` | `🐣 금일 {time} 신입 OT 진행합니다~` + 줄바꿈 + `장소 : {place}` | 진행할 때 문구 (`{time}`/`{place}` 치환) |
| `OT_DEADLINE_CLOSED_MESSAGE` | `🌙 오늘 신입 OT는 쉬어갑니다~` | 쉬어갈 때 문구 |
...
매주 **수요일 14:00(KST)** 에 `OT_NOTICE_CHANNEL_ID` 채널로 아래 임베드가 올라갑니다.
`참가` / `불참` 버튼이 함께 달립니다.

```
🐣 신입 오리엔테이션 수요조사
# 금일 9시 신입 오리엔테이션✨️
## 참가를 희망하시는 신입분🐣 or 한 번도 안 들어보신 분은 아래 참가 버튼을 눌러주세요!
...
누를 때마다 임베드의 명단이 갱신되고, **같은 버튼을 다시 누르면 응답이 취소**됩니다.
버튼 응답은 `ot_responses` 테이블에 공지 메시지별로 저장되므로 봇을 재기동해도
그대로 남고, 지난 주 응답이 이번 주에 섞이지 않습니다.

OT 는 **신입이거나 OT 를 한 번도 안 들어본 사람이 참가를 희망할 때만** 진행되므로,
최소 인원 같은 조건은 두지 않습니다. 대신 **미참가자가 1명이라도 `참가` 를 누르면**
그날 21시(`OT_SCHEDULE_HOUR`)로 OT 일정이 `/일정` 저장소에 자동 등록됩니다.

- **멘션 대상**: `참가` 를 누른 **사람 전원** (미참가자 + 이미 OT 를 들은 사람)
- **열지 말지**: **미참가자**가 한 명이라도 눌렀는지로만 판단 — 이미 들은 사람만
  눌렀다면 일정은 잡히지 않습니다.

응답을 취소하면 멘션 대상에서 빠지고, 미참가자가 한 명도 안 남으면 일정도 지워집니다.
봇이 꺼져 있는 동안 눌린 반응은 기동 시 공지의 반응 목록을 다시 읽어 반영합니다.

### 마감 공지 (19:00)

`OT_DEADLINE_HOUR` 시각에 **그때까지 모인 반응을 기준으로** 오늘 OT 를 여는지 한 번
공지합니다. 나가는 곳은 일정 알림 채널(`OT_DEADLINE_CHANNEL_ID`)입니다.

| 상황 | 공지 |
|---|---|
| 미참가자 반응 있음 | `🐣 금일 21시 신입 OT 진행합니다~` + 장소 + 참가자 멘션 |
| 미참가자 반응 없음 | `🌙 오늘 신입 OT는 쉬어갑니다~` |
| 그날 수요조사가 없었음 | 아무것도 보내지 않음 |

마감은 **공지 기준일 뿐 반응을 막지 않습니다.** 19시 이후에 눌러도 21시 일정의 멘션
대상에는 계속 반영됩니다(그 경우 공지 문구와 실제 멘션 인원이 다를 수 있습니다).
...
일정 시각이 되면 그 회차에 `참가` 를 누른 사람들은 자동으로 '참가함' 이 되어, 다음
주에는 그 사람들만으로 일정이 열리지 않습니다. 누르기만 하고 안 온 사람 등은
`/ot참가처리` 로 되돌립니다.
